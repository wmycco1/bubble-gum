...
**Версия:** 3.4.0 (God-Tier Development Protocol Added)
**Дата создания:** November 03, 2025
**Последнее обновление:** November 05, 2025
**Статус:** ✅ Production Ready - 25% Complete (Audited) | ⚠️ **Tech Stack Updated to Nov 2025**
...
**⚠️ CRITICAL: Прежде чем начать ЛЮБУЮ задачу, прочитай:**

### 📋 [God-Tier Development Protocol](./docs/GOD_TIER_PROTOCOL.md)

**Обязательная 5-фазная процедура для КАЖДОЙ задачи:**
1. **PHASE 0**: Understanding (понимание требований)
2. **PHASE 1**: Current State Verification (проверка текущего состояния)
3. **PHASE 2**: Detailed Planning (детальное планирование) - **WAIT FOR APPROVAL**
4. **PHASE 3**: Implementation (реализация)
5. **PHASE 4**: Comprehensive Verification (полная проверка)
6. **PHASE 5**: Final Report & Roadmap Update (отчёт и обновление roadmap)

**Стандарты качества:** TypeScript Strict, 60%+ Coverage, WCAG AA, OWASP Security, SOLID Principles

**Статус:** ✅ АКТИВЕН - Все задачи ДОЛЖНЫ следовать протоколу
...
8. **Git коммиты автоматически (когда логично):**
   - После завершения задачи/фичи → git commit
   - После создания файлов → git commit
   - После важных изменений → git commit
   - Формат: Conventional Commits (feat:, fix:, docs:, refactor:, test:, chore:)
   - ВСЕГДА проверять .env.local НЕ в git
   - Автоматический push после коммита: git push
   - Спрашивать разрешение ТОЛЬКО для критичных изменений (delete, force push)
   - **Когда коммитить:**
     * ✅ Создал новые файлы
     * ✅ Завершил фичу
     * ✅ Исправил баг
     * ✅ Обновил документацию
     * ❌ НЕ коммитить каждое мелкое изменение
9. **СООБЩАЙ** если обнаружишь противоречия или упущения в документации
10. **ПРЕДЛАГАЙ** улучшения если видишь лучший способ
11. **ИСПОЛЬЗУЙ ОБА источника документации + ПРОВЕРЯЙ АКТУАЛЬНОСТЬ:**
